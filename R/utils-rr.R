  ...
  .data <- inner_join(
    ...
    mutate(
      current_fraction_factory = pmap(
        list(target = incidence, mass = n_gamma, lb = threshold, ub = ub),
...
makeFreeFactories <- function(.data) {
  .data %>%
    ...
    mutate(
      binge_risk = pmap(
        list(im = im, bingef = bingef, extrapolated_risk = extrapolated_risk),
        makeBingeRisk
      )
    ) %>%
    mutate(
      current_fraction_factory = pmap(
        list(
          extrapolated_risk = extrapolated_risk,
